...
[**DeleteMediaFile**](MediaAPI.md#DeleteMediaFile) | **Delete** /media/{mediaId}/file | Delete media file
...
func main() {
	...
	createMediaByStatusRequest := *seerrClient.NewCreateMediaByStatusRequest() // CreateMediaByStatusRequest |  (optional)
...
> DeleteMediaFile(ctx, mediaId).Is4k(is4k).Execute()
...
func main() {
	...
	configuration := seerrClient.NewConfiguration()
	apiClient := seerrClient.NewAPIClient(configuration)
	r, err := apiClient.MediaAPI.DeleteMediaFile(context.Background(), mediaId).Is4k(is4k).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `MediaAPI.DeleteMediaFile``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
}
```
...
Other parameters are passed through a pointer to a apiDeleteMediaFileRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------

 **is4k** | **bool** | Whether to remove from 4K service instance (true) or regular service instance (false) | 
