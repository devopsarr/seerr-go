...
func main() {
	createRequestRequest := *seerrClient.NewCreateRequestRequest("movie", float32(123)) // CreateRequestRequest | 
...
> GetUserRequests2XXResponse GetRequest(ctx).Take(take).Skip(skip).Filter(filter).Sort(sort).SortDirection(sortDirection).RequestedBy(requestedBy).MediaType(mediaType).Execute()
...
func main() {
	...
	configuration := seerrClient.NewConfiguration()
	apiClient := seerrClient.NewAPIClient(configuration)
	resp, r, err := apiClient.RequestAPI.GetRequest(context.Background()).Take(take).Skip(skip).Filter(filter).Sort(sort).SortDirection(sortDirection).RequestedBy(requestedBy).MediaType(mediaType).Execute()
	if err != nil {
...
func main() {
	requestId := "1" // string | Request ID
	updateRequestRequest := *seerrClient.NewUpdateRequestRequest("MediaType_example") // UpdateRequestRequest | 
